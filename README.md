...
- **Bundle assembly:** the App Group root must contain files (not directories) named `geosite.json` and `geoip.mmdb`. `ForgeRuleCoreBundleError` distinguishes an unresolved container, a missing resource, malformed geosite data, and MMDB open failure. Resource versioning and atomic snapshot replacement remain host responsibilities.
